#!/usr/bin/env python
# coding: utf-8
print('➡️➡️➡️➡️➡️ BB PREDICTION started ⬅️⬅️⬅️⬅️⬅️')

# In[2]:
print("▶︎ ▶︎ ▶︎ PREDICTOR started ◀︎ ◀︎ ◀︎")


from bb_initial import engine,bb_selected_odds_list,bb_target_cols,bb_rond_threshold,bb_model_size
from bb_model_nb import model_names
...
df_unpred_cleaned_games = fn_read_data_db('BB_futureGames')
df_unpred_cleaned_games = df_unpred_cleaned_games.drop_duplicates(subset=['GameLink'], keep='last')

print(f'^^ pred data size before drop: {len(df_unpred_cleaned_games)}')
df_unpred_cleaned_games = df_unpred_cleaned_games.drop_duplicates(subset=['GameLink'],keep='last')
print(f'^^ pred data size after drop: {len(df_unpred_cleaned_games)}')
...
# loading of ML Models from local
models = {}
...
# sample usage > models['mdl_Linear_Reg_Dep_iy_score'].predict()



# new games cleaning
def game_cleaner(df):
    print('>> Games are being cleaned... << ')
...
def fn_prediction(df,response):
    ...
    df_pred_res = pd.DataFrame(columns=['MacTarihi','HomeTeam', 'AwayTeam', 'REGTYPE','Accuracy_%','Home_FT', 'Away_FT'])
    ...
    for idx, game_row in df.iterrows():
        ...
        gamelink = df_selectedTeam['GameLink'].iloc[0]
    ...
    df_pred_res[['Home_FT','Away_FT']] = df_pred_res[['Home_FT','Away_FT']].applymap(lambda x: 0 if x < 0 else x)
        
    df_pred_res['Total_Score'] = df_pred_res['Home_FT'] + df_pred_res['Away_FT']
    
    df_pred_res =df_pred_res[['MacTarihi', 'HomeTeam', 'AwayTeam', 'REGTYPE', 'Accuracy_%', 'Home_FT',
       'Away_FT', 'Total_Score','GameLink']]
    
    if response == 'yes' or response == 'Yes' or response == 'y' or response == 'YES' or response == 'Y':
        df_pred_res.to_sql(name='BB_Predicted_games_allModels', con=engine, if_exists='append', index=False)
    else:
        df_pred_res.to_sql(name='BB_Predicted_games_allModels', con=engine, if_exists='replace', index=False)
 
    print(f'\n** All selected games have been predicted with {len(model_names)} models and written into DB**')    
    return df_pred_res

def custom_round(n):
    if n - math.floor(n) >= bb_rond_threshold:
        return math.ceil(n)
    else:
        return math.floor(n)
...
def avg_calc(df,df_unpred_cleaned_games,response):
    ...
    df_avg = df_avg[['MacTarihi', 'HomeTeam','AwayTeam','REGTYPE','Accuracy_Mean','Home_FT_Mean','Away_FT_Mean']] #
    
    # cols to int from float by our threshold
    columns_to_update = ['Home_FT_Mean','Away_FT_Mean']
    ...
    print(f'** --> AVG SEC SIZE= {len(df_avg)} <-- **')
    ...
    if response == 'yes' or response == 'Yes' or response == 'y' or response == 'YES' or response == 'Y':
        df_avg.to_sql(name='BB_Predicted_games_AVG', con=engine, if_exists='append', index=False)
    else:
        df_avg.to_sql(name='BB_Predicted_games_AVG', con=engine, if_exists='replace', index=False)
    
    print('Has been predicted FOR SECONDARY LEAGUES')
    print(f"\n!!! All Model Accuracy Score= {round(np.mean(df_avg['Accuracy_Mean']),2)} #Selected ODDS LIST : - {bb_selected_odds_list[-1]} - !!! \n")
    ...
    log_dic = {'accuracy_mean':round(np.mean(df_avg['Accuracy_Mean']),2),
               'data_size': len(df)/bb_model_size, #13 for 13 model- can be changed later
               'Odds_List': bb_selected_odds_list[-1],
...
lst_gamelink = list(fn_read_data_db('BB_Predicted_games_AVG')['GameLink'])
# response = input('? Do you want to calculate only new games ? ')
if response == 'yes' or response == 'Yes' or response == 'y' or response == 'YES' or response == 'Y':
    df_cleaned_unpred = df_cleaned_unpred[~df_cleaned_unpred['GameLink'].isin(lst_gamelink)]
    print(f'^^ Only new game size to calculate > {len(df_cleaned_unpred)} ^^')
else:
    print(f'^^ Total game size to calculate > {len(df_cleaned_unpred)} ^^')
    


if len(df_cleaned_unpred) > 0:
    df_prediction = fn_prediction(df_cleaned_unpred,response)
    df_score_avg = avg_calc(df_prediction,df_cleaned_unpred,response)
else:
    print('There is NO new game to be calculated !')
    

print('⏮️⏮️⏮️⏮️⏮️ BB PREDICTION ended ⏭️⏭️⏭️⏭️⏭️')

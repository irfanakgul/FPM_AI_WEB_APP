print("▶︎ ▶︎ ▶︎ ANALYSIS STARTED ◀︎ ◀︎ ◀︎")

import sys
from initial import engine, init_version, \
    spreadsheet_id, model_inUse, model_list, week_high,week_low,week_threshold,\
...
def calc_winner_loser_leagues(df):
    ...
    df_perc.to_sql(name='WINNER_LOSER_LEAGUES_SORTED', con=engine, if_exists='replace', index=False)
    return df_perc
# %%
def fn_losers(df):
...
print('-----> STEP-2 | IM_CLEAN_SELECT.py started', flush=True)
# %%
...
df_final.to_sql(name='MK_REM_PRE_ELECTION_ALLMODEL', con=engine, if_exists='replace', index=False)
# %%
...
# ----------- calculate new Stake Comment ------------ #

def calculate_stake_comment(row, week_threshold,week,h_all_stand,a_all_stand,lg_size):
    ...
    # -------- A GRUBU --------
    def assign_A(start_no, min_percent):
        step = (100 - min_percent) / 5
        idx = int((a_percent - min_percent) // step)
        idx = min(idx, 4)
        return f"A-{start_no + idx}"

    if h_all_stand == 1 and a_percent >= 40:
        return assign_A(1, 40)

    if h_all_stand == 2 and a_percent >= 50:
        return assign_A(6, 50)

    if h_all_stand == 3 and a_percent >= 70:
        return assign_A(11, 70)
...
def fn_selection_sec(df,version,week_threshold):
    ...
    for idx, game_row in df.iterrows():
        ...
        if int(week) > week_threshold:
        
            df_single['StakeComment'] = df_single.apply(
    ...
    df_results.to_sql(name=f'LOG_MK_IM_STAKER_ALL_COMMENT_ALLMODEL_ALL_TIME', con=engine, if_exists='append', index=False)   
    
    df_results.to_sql(name=f'MK_IM_STAKER_ALL_COMMENT_ALLMODEL', con=engine, if_exists='replace', index=False)   

    size_A_B = len(df_results[df_results['StakeComment'].str.contains(r'(A-|B-)', na=False)])
    print(f'** {version} MK_IM_STAKER_ALL_COMMENT_ALLMODEL | Size A-B Comments: {size_A_B}')    
...
df_semi_final =df_semi_final[df_semi_final['RISK_Value']<init_RISK_VALUE_threshold] # from initial file 0-1
...
def fn_customer_selection(df):
    # MacTarihi	Saati	Lig	EvSahibi	KonukEkip	Secim	Ms1	Ms2	RunTime	STATUS	GameLink
    sel_cols = ['MacTarihi','Time', 'Lig', 
        'EvSahibi','KonukEkip', 'MS_SELECTION', 'Ms1', 'Ms2','period','RunTime','STATUS','GameLink']
    
    df = df[sel_cols]
    
    fn_write_to_google(df,spreadsheet_id,'CUSTOMER',replace_or_append = 'replace',path=GOOGLE_API_PATH)
    
    df.to_sql(name='CUSTOMER', con=engine, if_exists='append', index=False)
    print('-----> CUSTOMER games has been calculated into gg and db! <------')


fn_customer_selection(df_final)

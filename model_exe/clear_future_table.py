#!/usr/bin/env python3
import sys,re
from initial import engine, lst_clear_button_tables,lst_protected_tables
import pandas as pd


if len(sys.argv) == 1:
    print(f"[QUESTION] ? Are you sure to clean table: {lst_clear_button_tables} ? ", flush=True)
...
if IS_SURE.lower() == "confirm":
    for table in lst_clear_button_tables:
        if table not in lst_protected_tables:
    ...
    print(f"✅ [SYSTEM] ALL TABLES CLEANED: {lst_clear_button_tables}✅", flush=True)

else:
    print(f"❌ [SYSTEM] Table cleaning cancelled", flush=True)

...
// const DB_FILE = "/Users/irfanakgul/Desktop/FPM_AI_WEB_ALL/model_exe/database/results.db";
...
// sadece harf-rakam-altçizgi izinli
...
function getTableColumns(db, table) {
  return new Promise((resolve, reject) => {
    db.all(`PRAGMA table_info(${table})`, (err, rows) => {
      ...
      resolve(rows.map(r => r.name)); // sütun isimleri
    });
  });
}
...
app.get("/api/db/read", async (req, res) => {
  ...
  try {
    // sütunları çek (boş tablo olsa bile column gelsin)
    ...
    for (const c of cols) {
      const qv = req.query[`q_${c}`];
      if (typeof qv === "string" && qv.trim()) {
        where.push(`${c} LIKE ?`);
        params.push(`%${qv.trim()}%`);
      }
    }
    ...
    const total = await new Promise((resolve, reject) => {
      db.get(`SELECT COUNT(*) AS cnt FROM ${table} ${whereSql}`, params, (err, row) => {
        if (err) return reject(err);
        resolve(row.cnt || 0);
      });
    });
    ...
    const rows = await new Promise((resolve, reject) => {
      db.all(
        `SELECT rowid AS __rowid, * FROM ${table} ${whereSql} LIMIT ? OFFSET ?`,
        [...params, limit, offset],
        (err, rs) => {
          if (err) return reject(err);
          resolve(rs);
    ...
    db.close();
    res.json({ columns: ["__rowid", ...cols], rows, total });
  } catch (e) {
    db.close();
...
const PROTECTED_TABLES = ["results", "tbl_futureGames", "only_links_all","MK_League_List","_ONLY_SCORES",
    "MK_WEEKLY_STANDINGS_rawContainer","tbl_future_all_collect_RAW","disabledGameLinks","log_TimeJump",
    "selection_Driver_List","MK_WEEKLY_STANDINGS","LEGACY_1_FOCUS_ALL_byMAY2025"
];
...
app.delete("/api/db/table/:name", (req, res) => {
  ...
  if (PROTECTED_TABLES.includes(table)) {
    return res.status(403).json({ error: "This table is protected" });
  }
...
app.post("/api/db/update", async (req, res) => {
  ...
  // changes: [{ __rowid: 123, colA: "x", colB: "y" }, ...]
  ...
  try {
    ...
    for (const row of changes) {
      ...
      const keys = Object.keys(row).filter(k => k !== "__rowid" && colSet.has(k));
      ...
      await new Promise((resolve, reject) => {
        db.run(
          `UPDATE ${table} SET ${setSql} WHERE rowid = ?`,
          [...vals, rowid],
